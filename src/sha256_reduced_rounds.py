...
class SHA256:
    ...
    def sha256_compress(self, words):
        ...
        for i in range(self.rounds):
            ...
            print(i, "=",hex(a)," ", hex(b)," ",hex(c), " ",hex(d), " ", hex(e)," ", hex(f), " ", hex(f), " ", hex(g), " ", hex(words[i]))
            h = g
            g = f
            f = e
        ...
        print(self.rounds, "=",hex(a)," ", hex(b)," ",hex(c), " ",hex(d), " ", hex(e)," ", hex(f), " ", hex(f), " ", hex(g), " ", hex(words[self.rounds]))

        self.H = (self.H + [a, b, c, d, e, f, g, h]) & 0xFFFFFFFF

    def calculate_hash(self, input_words):
        self.sha256_compress(np.array(input_words, dtype=np.uint32))
        return ''.join(f'{value:08x}' for value in self.H)

if __name__ == "__main__":
    sha = SHA256(rounds=3)
    #input_words = [0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8,
    #               0x9, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16]
    #input_words = [0xbba73b74, 0xaa8d8059, 0x0e4d263c, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]
    input_words = [0x27ac9946, 0xaa8d8059, 0x0e4d263c, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]


    hash_result = sha.calculate_hash(input_words)
    print("Step-Reduced SHA-256 Hash:", hash_result)
